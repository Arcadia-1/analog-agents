...
name: analog-design
description: >
  Transistor-level circuit design for one analog sub-block.
  Produces Spectre netlist with hand-calculation rationale.
  Use when designing a specific circuit block after architecture is defined.
---
...
Read the active effort level at startup. Print it:
    ...
    [effort: <level>] max_iter=<N>, wiki=<mode>

### Iteration Limits by Effort

| Effort | Max iterations | Special behavior |
|--------|---------------|-----------------|
| lite | 2 | — |
| standard | 3 | — |
| intensive | 5 | — |
| exhaustive | unlimited | Optimizer auto-triggers on iteration 2+ when margins are tight |

See `shared-references/effort-contract.md` for the full dimension table and invariant rules.

## Wiki Interaction

At the start of each block design, call `/analog-wiki consult` with the block type
to retrieve sizing strategies, anti-patterns, and historical cases from the knowledge
graph. Incorporate relevant findings into your sizing rationale.
...
Example: Mbn1 (W=1u, nf=1, L=120n) carries 20uA, finger width = 1u.
M3 (W=8u, nf=8, L=120n) → ratio = 8:1 → ~160uA, finger width = 1u → 匹配好。

### Spectre Netlist Pitfalls (from experience)

**1. isource direction**
`I0 (A B) isource dc=20u` means 20uA flows from A to B.
...
(phase is flat 0 deg), the PSF complex data is not being handled correctly.
...
gm = (gm/Id) x Id. Look up or estimate W needed for target gm/Id at given L and Id.
...
At **exhaustive** effort, the optimizer auto-triggers on iteration 2+ when margins are tight.
...
After dispatching the verifier, suggest running `/analog-review run` for cross-model
circuit audit (skipped at lite effort, see `shared-references/effort-contract.md`).
...
- **Be actionable on failure**: "Increased Cc from 2p to 2.8p to recover 4 deg phase margin at cost of -8MHz UGBW"
